...
<body class="is-preload">
 ...
 %>
							...
							<li class="viewBoardProfile2">
								...
								if (((MemberVO) session.getAttribute("vo")).getNick().equals(view.getNick())) {
								%>
								<a href="DeleteService?pnum=<%=view.getPnum()%>"
 ...
 %>
				...
				<form action="GoodInsertService" method="post"
					...
					<input name="id" type="hidden" value="<%=vo.getId()%>"> <input
						type="image" src="image/like.png" class="like_icon" alt="제출버튼"
						style="width: 3%">
					<%=view.getGood()%>
					<br>
				</form>
			...
			<div class="comments">
				...
				<ul class="myInfo" id="commentUl">
					...
					<div class="commentBox"
						...
						<li>
							<h3 class="medium">
								<img alt="profile" src="image/profile.png" class="pro">
								<%=cvo.getNick()%></h3>
						</li>
						...
						<p class="medium22"><%=cvo.getCdate()%>
						</p>


						<a href="#" class="vote231"><img src="image/like.png"
							class="like_icon"> 0</a>
						...
						%>
						<a
							href="ComDeleteService?cnum=<%=cvo.getCnum()%>&pnum=<%=view.getPnum()%>"
							class="charRed">삭제</a> <a href="#" class=" size21">공감</a>
						...
						<form action="GoodInsertService" method="post"
							...
							<input type="image" src="image/like.png" class="like_icon"
								alt="제출버튼" style="width: 3%">
							<%=view.getGood()%>
							<br>
						</form>
			...
			<div class="col-12" style="padding-top: 1.5em">
				<form action="ComInsertService" method="post" class="writercomment">
					<div class="col-12">
						<td><input name="pnum" type="hidden"
							value="<%=view.getPnum()%>"> <input name="id"
							type="hidden" value="<%=vo.getId()%>"> <input name="nick"
							type="hidden" value="<%=vo.getNick()%>"></td>
						<tr>
							<td colspan="2"><textarea name="contents" id="demo-message"
									placeholder="댓글을 입력하세요." rows="6"></textarea></td>
						</tr>
					</div>
					<div class="col-6 col-12-small align-right">
						<input type="checkbox" id="demo-copy" name="demo-copy"> <label
							for="demo-copy">익명</label> <input type="submit"
							class="button primary buttonSize" value="댓글등록">
					</div>
